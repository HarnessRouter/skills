...
**`add` takes properties with `--prop`, not bare flags.** `--text` is accepted and silently ignored:

```bash
officecli add report.docx /body --type p --text "Q3 Review"        # WRONG — empty paragraph
officecli add report.docx /body --type p --prop text="Q3 Review"   # correct
```

The wrong form still prints `Added p at /body/p[...]`, so the failure only shows up when someone
opens the document and finds it blank. Check with `officecli view FILE text` before delivering.

**A freshly created document has no built-in styles.** `--prop style=Heading1` on a document made
